...
/// @author nani.eth (https://github.com/NaniDAO/accounts/blob/main/src/AccountURL.sol)
contract AccountURL is ERC4337 {
        ...
        returns (bytes4)
    {
        ...
        else return AccountURL(payable(validator)).isValidSignature(hash, signature);
    }
        ...
        returns (bytes memory result)
    {
        ...
        if (balanceAfter * 100 / balanceBefore < rugPercent) revert Unauthorized();
    }
    ...
    /// @dev Locking function where user `owner()` or EntryPoint can set call `rugPercent`.
    function setRugPercent(uint256 _rugPercent) public payable onlyEntryPointOrOwner {
        rugPercent = _rugPercent;
    }
}
