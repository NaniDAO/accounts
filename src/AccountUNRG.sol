...
/// @notice Simple unruggable smart account implementation. Includes plugin tooling.
/// @author nani.eth (https://github.com/NaniDAO/accounts/blob/main/src/AccountUNRG.sol)
contract AccountUNRG is ERC4337 {
    ...
    /// @dev Unrug percentage.
    /// Default to ten percent.
    uint256 public unrugPercent = 10;
        ...
        returns (bytes4)
    {
        ...
        else return AccountUNRG(payable(validator)).isValidSignature(hash, signature);
    }

    /// @dev Execute a guarded call from this account.
        ...
        returns (bytes memory result)
    {
        ...
        if (balanceAfter >= balanceBefore) return result;
        unchecked {
            if ((balanceBefore - balanceAfter) > (balanceBefore * unrugPercent / 100)) {
                revert Unauthorized();
            }
        }
    }

    /// @dev Execute a sequence of guarded calls from this account.
        ...
        returns (bytes[] memory results)
    {
        ...
        uint256 balanceAfter = address(this).balance;
        if (balanceAfter >= balanceBefore) return results;
        unchecked {
            if ((balanceBefore - balanceAfter) > (balanceBefore * unrugPercent / 100)) {
                revert Unauthorized();
            }
    ...
    /// @dev Execute a guarded delegatecall with `delegate` on this account.
    ...
    {
        uint256 balanceBefore = address(this).balance;
        result = delegateExecute(delegate, data);
        uint256 balanceAfter = address(this).balance;
        if (balanceAfter >= balanceBefore) return result;
        unchecked {
            if ((balanceBefore - balanceAfter) > (balanceBefore * unrugPercent / 100)) {
    ...
    /// @dev Locking function where user `owner()` or EntryPoint can set call `unrugPercent`.
    function setUnrugPercent(uint256 _unrugPercent) public payable onlyEntryPointOrOwner {
        unrugPercent = _unrugPercent;
    }
}
